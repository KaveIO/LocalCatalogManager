...
public class BackendHiveImplIntTest {
  ...
  public static void setUpClass() throws IOException, SQLException, BackendException {
    ...
    try (Connection con = DriverManager.getConnection(server, user, passwd)) {
      ...
      stmt.execute(sql);
      sql = "INSERT INTO TABLE default.lcm_test VALUES " + "(\"Val1\",\"Val2\",\"Val3\",\"Val4\"), "
          + "(\"Val5\",\"Val6\",\"Val7\",\"Val8\"), " + "(\"Val9\",\"Val10\",\"Val11\",\"Val12\")";
      stmt.execute(sql);
      ...
      sql = "INSERT INTO TABLE default.lcm_test_delete VALUES"
          + "(\"Wrong1\",\"Val2\",\"Val3\",\"Val4\"), " + "(\"Val5\",\"Val6\",\"Val7\",\"Val8\"), "
  ...
  /**
   * Test of getSupportedUriSchema method, of class BackendHiveImpl. Method returns supported uri
   * schema ("hive") and tests fails if it is not the case.
   *
   */
  @Test
  ...
  /**
   * Test of connection to the hive server. Method connects to the hive2 server specified in
   * "storagePath" of backendModel. It assumes existence of a database called "default" and lists
   ...
   *         {@link Storage} instance that initialized the {@link BackendHiveImp} instance.
   */
  @Test
  ...
  /**
   * Tests what happens if {@link BackendHiveImp} gathers information using {@link MetaData} object
  ...
  /**
   * Tests what happens if {@link BackendHiveImp} gathers information using empty {@link MetaData}
  ...
  /**
   * Tests what happens if {@link BackendHiveImp} gathers information using {@link MetaData} object
   * with invalid URI. The method should fail as well because invalid uri scheme is supplied.
   *
  ...
  /**
   * Tests what happens if {@link BackendHiveImp} gathers information using {@link MetaData} object
   * with valid but incomplete URI. The method should fail as well because the URI is not complete
   *
  ...
  /**
   * Tests store() method of {@link BackendHiveImp}. Test tries to store text file created during
   * setup as a table in hive, then it queries its content and saves it to local text file and
   * finally tests if the new text file is identical to the original using md5.
   *
  ...
  /**
   * Tests delete() method of {@link BackendHiveImp}. It tries to delete table created in setup. It
   * fails if it is not possible to.
   *
