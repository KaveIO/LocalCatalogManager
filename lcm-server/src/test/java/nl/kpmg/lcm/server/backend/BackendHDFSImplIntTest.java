...
/**
 * Test suite for HDP 2.2 Hortonworks Sandbox. The namenode is running on port 8020. If using
 * VirtualBox, make sure that the guest machine network interface is attached to "host-only adapter"
 * . The IP of the guest machine can be found using ifconfig command.
 *
 * @see <a href=
 *      "https://gitlab-nl.dna.kpmglab.com/kave/localcatalogmanager/wikis/accessing-sandbox"> wiki
 *      </a>
 *
 *
...
public class BackendHDFSImplIntTest {
  ...
  public static void setUpClass() throws Exception {
    ...
    Process p;
    try {
      p = Runtime.getRuntime().exec("hdfs dfs -mkdir " + TEST_STORAGE_PATH + "user/test");
      p.waitFor();
      p = Runtime.getRuntime().exec("hdfs dfs -copyFromLocal " + TEST_DIR + "/testFile.csv "
          + TEST_STORAGE_PATH + "user/test");
      p.waitFor();
    } catch (IOException | InterruptedException ex) {
      Logger.getLogger(BackendHDFSImpl.class.getName()).log(Level.SEVERE,
          "Cannot access the hdfs at " + TEST_STORAGE_PATH, ex);
    }
    if (mkdir) {
  ...
  public static final void tearDownClass() {
    ...
    Process p;
    try {
      p = Runtime.getRuntime().exec("hdfs dfs -rm " + TEST_STORAGE_PATH + "user/test/*");
      p.waitFor();
      p = Runtime.getRuntime().exec("hdfs dfs -rmdir " + TEST_STORAGE_PATH + "user/test");
      p.waitFor();
    } catch (IOException | InterruptedException ex) {
      Logger.getLogger(BackendHDFSImpl.class.getName()).log(Level.SEVERE,
          "Cannot access the hdfs at " + TEST_STORAGE_PATH, ex);
  ...
  /**
   * Tests what happens if {@link BackendHDFSImp} gathers information using empty {@link MetaData}
  ...
  /**
   * Tests what happens if {@link BackendHDFSImp} gathers information using {@link MetaData} object
   * with invalid URI.
   *
   * @throws BackendException if empty metadata are supplied.
   */
  ...
  /**
   * Tests store() method of {@link BackendHDFSImp}. Test tries to store text file created during
   * setup, then it downloads back and finally tests if it is identical to the original using md5.
   *
   * @throws BackendException if it is not possible to gather information about the dataset
  ...
  public final void testStore() throws IOException, BackendException {
    ...
    try {
      p = Runtime.getRuntime().exec("hdfs dfs -copyToLocal " + TEST_STORAGE_PATH
          + "user/test/testStore.csv " + TEST_DIR + "/testStore.csv");
      p.waitFor();
    } catch (IOException | InterruptedException ex) {
      Logger.getLogger(BackendHDFSImpl.class.getName()).log(Level.SEVERE,
          "Cannot access the hdfs at " + TEST_STORAGE_PATH, ex);
  ...
  /**
   * Tests read() method of {@link BackendHDFSImp}. Test reads a text file created by setup and
   * stores in the new text file. Then it tests if the 2 files are identical using md5.
   *
   * @throws IOException if there are problems with creating local file or writing to it
  ...
  public final void testRead() throws IOException, BackendException {
    ...
    try (InputStream is = testBackend.read(metaData)) {
      try (FileOutputStream fos = new FileOutputStream(output)) {
        ...
        Logger.getLogger(BackendHDFSImpl.class.getName()).log(Level.INFO, "{0} bytes read",
            readBytes);
        fos.flush();
      }
    }
  ...
  /**
   * Tests delete() method of {@link BackendHDFSImp}. It tries to store a file on HDFS backend and
   * then delete it. It fails if it is not possible to delete the file.
   *
  ...
  public final void testDelete() throws BackendException {
    ...
    try {
      p = Runtime.getRuntime().exec("hdfs dfs -copyFromLocal " + TEST_DIR + "/testFile.csv "
          + TEST_STORAGE_PATH + "user/test/testDelete.csv");
      p.waitFor();
    } catch (IOException | InterruptedException ex) {
      Logger.getLogger(BackendHDFSImpl.class.getName()).log(Level.SEVERE,
          "Cannot access the hdfs at " + TEST_STORAGE_PATH, ex);
