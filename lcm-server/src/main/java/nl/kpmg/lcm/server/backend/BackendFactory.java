...
public class BackendFactory {
    ...
    public Backend createBackend(String sourceType, Storage storage) throws BackendNotImplementedException {
        ...
        if (sourceType == null || sourceType.isEmpty() || sourceType.length() > MAX_KEY_LENGTH) {
            throw new IllegalArgumentException("Schame parameter could not be null or empty! The maximun length is: " + MAX_KEY_LENGTH);
        }
        ...
        Backend backendImpl = getBackendImplementation(backendClass, storage);
        return backendImpl;
    }
    ...
    private void scan() {
        ...
        for (BeanDefinition bd : scanner.findCandidateComponents(backendSourcePackage)) {
            ...
            if (backendClass == null) {
                logger.log(Level.WARNING, "Found class with scanner: {0} but was not able to laod it", new Object[]{bd.getBeanClassName()});
                continue;
            }
    ...
    private Backend getBackendImplementation(Class<?> clazz, Storage storage) {
        try {
            Constructor<?> constructor = clazz.getConstructor(Storage.class);
            Backend backendImpl = (Backend) constructor.newInstance(new Object[]{storage});

            return backendImpl;
        } catch (NoSuchMethodException ex) {
            logger.log(Level.WARNING, "Unable to find a constructor with Storage class as paraqmeter ", ex);
