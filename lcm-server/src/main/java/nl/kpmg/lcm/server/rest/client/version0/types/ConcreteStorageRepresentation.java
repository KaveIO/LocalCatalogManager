...
public class ConcreteStorageRepresentation extends StorageRepresentation implements LinkInjectable {

  @InjectLinks({@InjectLink(resource = StorageController.class, style = InjectLink.Style.ABSOLUTE,
      rel = "self", method = "getBackend",
      bindings = {@Binding(name = "backendName", value = "${instance.item.id}")})})
  @JsonIgnore
  private List<Link> injectedLinks;

  public ConcreteStorageRepresentation() {}

  public ConcreteStorageRepresentation(Storage storage) {
    setItem(storage);
  }
