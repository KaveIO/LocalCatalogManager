 ...
 * CoreTask and EnrichmentTask. Currently this uses the singleton design pattern. It is likely this
...
public final class TaskManager {
   ...
   * up to date and running. Currently two core tasks are scheduled. ExecuteTasksCoreTask Which will
   * find tasks with a PENDING state in the task list and execute them. LoadScheduleCoreTask Which
  ...
  public void initialize(ApplicationContext context) throws TaskManagerException {
    try {
      if (scheduler == null || !scheduler.isStarted()) {
        ...
        scheduleCoreTask("executeTasksCoreTask", ExecuteTasksCoreTask.class,
            EXECUTE_CORE_TASK_CRON_SCHEDULE);
        scheduleCoreTask("loadScheduleCoreTask", LoadScheduleCoreTask.class,
            LOAD_CORE_TASK_CRON_SCHEDULE);
   ...
   * LoadScheduleCoreTask but in most other cases direct access to the scheduler is at least
   * suspicious.
   *
