 ...
 *
 * This class converts DBObjects to User Objects at the moment these are loaded
 * from the database. All passwords which are loaded from the database are hashed
...
public class MongoUserReadConverter implements Converter<DBObject, User> {
    ...
    public User convert(DBObject source) {
        try {
            ...
            Logger.getLogger(MongoUserReadConverter.class.getName())
                    .log(Level.SEVERE, "Couldn't load user", ex);
            return null;
        }
    }
}
