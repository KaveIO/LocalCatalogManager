...
public class UserControllerClientTest extends LCMBaseServerTest {
  ...
  public void beforeTest() {
    ...
    Response res = target.path("client/login").request().post(entity);
    authenticationToken = res.readEntity(String.class);
  }

  @After
  public void afterTest() {
    target.path("client/logout").request().header("LCM-Authentication-User", "admin")
  ...
  public void testGetUserTarget() throws LoginException, UserPasswordHashException {
    ...
    Response response = target.path("client/v0/users/" + saved.getId()).request()
  ...
  public void testGetUsersWebTarget() throws LoginException {
    ...
    Response res1 =
        target.path("client/v0/users").request().header("LCM-Authentication-User", "admin")
            .header("LCM-Authentication-Token", authenticationToken).get();
  ...
  public void testModifyUserWebTarget() throws LoginException, UserPasswordHashException {
    ...
    Response res1 = target.path("client/v0/users/" + saved.getId()).request()
        .header("LCM-Authentication-User", "admin")
  ...
  public void testDeleteUserWebTarget() throws LoginException, UserPasswordHashException {
    ...
    // Delete the user
    response = target.path("client/v0/users/" + saved.getId()).request()
        .header("LCM-Authentication-User", "admin")
