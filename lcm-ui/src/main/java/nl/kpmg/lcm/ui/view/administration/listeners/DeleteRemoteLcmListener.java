 ...
 * @author shristov
 */
public class DeleteRemoteLcmListener extends AbstractListener {
  private static final org.slf4j.Logger LOGGER = LoggerFactory
      .getLogger(DeleteRemoteLcmListener.class.getName());

  /**
   * @param dataContainer parent view.
   */
  public DeleteRemoteLcmListener(final DynamicDataContainer dataContainer,
  ...
  public void buttonClick(final Button.ClickEvent event) {
    ...
    String message =
        String.format("Are you sure that you want to delete %s with name: \"%s\"?", getItemName(),
            item.getName());
          ...
          public void onClose(ConfirmDialog dialog) {
            if (dialog.isConfirmed()) {
              try {
                ...
                String notMessage =
                    String.format("Unable to delete %s with name:  %s!", getItemName(),
                        item.getName());
